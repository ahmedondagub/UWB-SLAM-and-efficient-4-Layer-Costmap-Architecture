...
The mission UI is mandatory for both active modes, and RViz is the spatial selector.
Use `2D Goal Pose` for a single target and `Publish Point` four times for a coverage box.
...
The main entry point is [launch/uwb_simulation.launch.py](launch/uwb_simulation.launch.py).
...
1. Save the generated files as [maps/warehouse.yaml](maps/warehouse.yaml) and [maps/warehouse.pgm](maps/warehouse.pgm).
2. Rebuild the package so the map files are installed into the package share directory.
3. Launch the stack normally.
...
This starts the warehouse simulator, the mission UI, Nav2, the Nav2 smoother, and the mission orchestration layer.
The operator clicks the warehouse in RViz to choose targets, and the selected mission is highlighted with marker overlays.
...
For RViz, use [config/uwb_slam.rviz](config/uwb_slam.rviz) and verify the map layers, TF tree, pose, and robot model.

### MATLAB fallback analysis

If Gazebo or the full simulation stack is unstable, use the MATLAB logging and plotting scripts in [matlab/](matlab/) to capture live ROS 2 telemetry and generate report figures offline.
...
- Main launch file: [launch/uwb_simulation.launch.py](launch/uwb_simulation.launch.py)
- Mission UI: [uwb_slam/mission_ui_node.py](uwb_slam/mission_ui_node.py)
- Motion coordinator: [uwb_slam/autonomous_motion_node.py](uwb_slam/autonomous_motion_node.py)
- Pose fusion: [uwb_slam/eskf_fusion_node.py](uwb_slam/eskf_fusion_node.py)
